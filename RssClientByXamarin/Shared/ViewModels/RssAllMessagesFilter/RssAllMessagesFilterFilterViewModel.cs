...
namespace Shared.ViewModels.RssAllMessagesFilter
{
    public class RssAllMessagesFilterFilterViewModel : ViewModel
    {
        ...
        private void DoSetMessageFilterType(MessageFilterType type)
        {
            UpdateFilter((filter) => { filter.MessageFilterType = type; });
        }
        
        private void DoSetFromDate(DateTime fromDate)
        {
            UpdateFilter((filter) => filter.From = fromDate);
        }
        
        private void DoSetToDate(DateTime toDate)
        {
            UpdateFilter((filter) => filter.To = toDate);
        }
